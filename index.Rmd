---
title: "NDAB02 Material"
author: "Isak Hietala"
date: "2021-04-26, ver 0.4"
output: 
  bookdown::gitbook:
    config:
      toc:
...
knitr::opts_chunk$set(echo = TRUE, fig.height = 2, fig.align = "center", fig.width = 6, eval = TRUE, message = FALSE)
...
## Regressionsmodeller
När vi vill undersöka sambandet mellan två kontinuerliga variabler kan vi använda linjära regressionsmodeller. Exempelvis skulle vi vilja vara intresserade av att undersöka sambandet mellan `Petal.Width` och `Petal.Length` ur `iris` data (se \@ref(tab:iris)). Vi har i kapitel \@ref(spridningsdiagram) redan visat hur ett spridningsdiagram kan skapas för att visuellt undersöka hur sambandet ser ut.  
...
ggplot(iris) + aes(x = Petal.Width, y = Petal.Length) + geom_point() + 
  ...
  theme(axis.title.y = 
        ...
        panel.grid.minor.y = 
          element_line(color = "light gray")) + 
  labs(title = "Sambandet mellan längd och bredd på blomblad",
       x = "Bladbredd",
       y = "Bladlängd") + 
...
För att matematiskt skatta den regressionsmodell som används för att rita ut linjen i ovanstående diagram använder vi oss utav `lm()` som står för linear model. Även denna funktion använder sig utav argumenten:

- `formula` - anger strukturen av modellen enligt `beroende ~ oberoende`,
- `data` - anger vilket datamaterial som ska analyseras.

```{r}

modell <- lm(formula = Petal.Length ~ Petal.Width, data = iris)

```

På samma sätt som i ANOVA, måste vi använda `summary()` för att få en mer detaljerad utskrift.

```{r}
summary(modell)
```

Den tredje och fjärde delen av denna utskrift är vad som vi är intresserade av, där `coefficients` innehåller alla skattade parametrar i modellen, och den sista delen innehåller utvärderingsmått och en kortfattad sammanfattning av regressionens ANOVA-tabell. 

Ur kolumnen `Estimate` kan vi se att parameterskattningen för $\alpha$ har blivit `r modell$coefficients[1] %>% round(2)`, och skattningen för $\beta$ blivit `r modell$coefficients[2] %>% round(2)`. 

### Inferens i regression
Enskilda t-test för lutningsparametrar syns i `coefficients`, och p-värden kan utläsas från kolumnen `Pr(>|t|)`. 

F-testet för hela modellens signifikans visas i den sista delen där `F-statistic` anger testvariabeln och `p-value` anger p-värdet för testet.